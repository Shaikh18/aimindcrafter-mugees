    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card overflow-hidden border-0">
                <div class="card-body">
                    <form action="{{route('admin.page-builder.update', $page_builder->id)}}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-12">
                                <label for="">Page Title <span class="text-danger font-weight-bold">*</span></label>
                                <input type="text" class="form-control" name="title"
                                       value="{{$page_builder->title ?? ''}}" readonly>
                                @error('title')
                            ...
                            <div class="col-md-12 mt-5">
                                ...
                                <select class="form-control @error('position') is-invalid @enderror" name="position">
                                    ...
                                    <option value="header" {{$page_builder->position == 'header' ? 'selected' : ''}}>Header</option>
                                    <option value="footer" {{$page_builder->position == 'footer' ? 'selected' : ''}}>Footer</option>
                                </select>
                                @error('position')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
